...
function Modal({ children, open, setOpen }: modalProps) {
    ...
    return (
        <AnimatePresence mode="sync">
            {open &&
                <motion.div onClick={() => setOpen(false)} className="h-screen w-screen flex items-center justify-center fixed top-0 right-0" initial={{ backgroundColor: 'rgba(0,0,0,0)' }} animate={{ backgroundColor: 'rgba(0,0,0,0.5)' }} exit={{ backgroundColor: 'rgba(0,0,0,0)' }} transition={{ duration: 0.5 }}>
